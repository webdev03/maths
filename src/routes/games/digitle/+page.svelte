...
<p>This game is similar to the popular word game Wordle. In this game, rather than using the alphabet to construct words, you use the digits 0-9 to find the missing number.</p>
<p>First, you have to place a guess and submit it. Then, a row of digit will appear. If a digit is <span class="text-green-400">green</span>, the target number has a digit in the same place with the same value. If a digit is <span class="text-yellow-400">yellow</span>, then the target number has a digit with the same value, but in a different place. Otherwise, the digit does not appear in the target number.</p>
